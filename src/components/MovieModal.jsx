        ...
        <div className="p-6 overflow-y-auto">
          <div className="flex flex-col md:flex-row gap-6 mb-6">
            ...
            <div className="flex-grow">
              ...
              {user && ( <button onClick={handleFavoriteClick} className={`mb-4 px-4 py-2 rounded-lg font-semibold transition-colors ${isFavorited ? 'bg-red-600 hover:bg-red-700' : 'bg-cyan-500 hover:bg-cyan-600'}`}> {isFavorited ? '❤️ Remove from Favorites' : '🤍 Add to Favorites'} </button> )}
              {isLoading ? ( <p className="text-gray-400">Loading details...</p> ) : details ? ( <> <p className="mb-4 text-gray-400 italic">{details.tagline}</p> <p className="mb-4 text-gray-300">{details.overview}</p> <div className="flex flex-wrap gap-x-4 gap-y-2 mb-4 text-sm"> <span><strong>Rating:</strong> ⭐ {details.vote_average?.toFixed(1)} / 10</span> <span><strong>Runtime:</strong> ⏳ {details.runtime} minutes</span> <span><strong>Released:</strong> 🗓️ {details.release_date}</span> </div> <div className="flex flex-wrap gap-2 items-center"> <strong>Genres:</strong> {details.genres?.map(genre => ( <span key={genre.id} className="bg-gray-700 rounded-full px-3 py-1 text-sm"> {genre.name} </span> ))} </div> </> ) : ( <p className="text-red-500">Could not load movie details.</p> )}
            </div>
          </div>
